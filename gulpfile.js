...
function sassDev() {
    return src(config.sass.src, { since: lastRun(sassDev), sourcemaps: true })
...
function sassPrd() {
    return src(config.sass.src, { since: lastRun(sassPrd) })
        .pipe(sass({ outputStyle: 'compact' }).on('error', sass.logError))
        .pipe(autoprefixer())
        .pipe(csso())
...
function css() {
    return src(config.css.src, { since: lastRun(css) })
        // .pipe(autoprefixer())
        // .pipe(csso())
...
function js() {
    return src(config.js.src, { since: lastRun(js) })
...
function watchingDev() {
    ...
    watch([config.template.parts, config.src + '/template/layer/**'], templateAll);
...
exports.serve = parallel(series(parallel(template, templateM), sassDev, js, img, etc, bSync), watchingDev);
exports.build = parallel(series(parallel(template, templateM), sassPrd, js, img, etc, bSync), watchingPrd);
